...
class Solver(object):
    def __init__(self, hyp_params, train_loader, dev_loader, test_loader, is_train=True, model=None, pretrained_emb=None):
        ...
        print("Loss function set to L1 Loss.")
        ...
        if self.is_train:
            ...
            print(f"Parameters assigned: {len(mmilb_param)} MMILB params, {len(main_param)} Main params, {len(bert_param)} BERT params.")

            for p in (mmilb_param + main_param):
                if p.dim() > 1:  # Apply Xavier initialization only to tensors with more than 1 dimension
                    nn.init.xavier_normal_(p)
                    print(f"Xavier initialization applied to param: {p.size()}")
    ...
    def train_and_eval(self):
        ...
        def train(model, optimizer, criterion, stage=1):
            ...
            start_time = time.time()
            left_batch = self.update_batch
            mem_size = 1  # Entropy estimate interval
            mem_pos_tv, mem_neg_tv, mem_pos_ta, mem_neg_ta = [], [], [], []

            print(f"Starting training for stage {stage}. Total batches: {num_batches}")
            ...
            for i_batch, batch_data in enumerate(self.train_loader):
                ...
                print(f"Batch {i_batch + 1}/{num_batches} | Memory initialized.")
                ...
                if stage == 1:
                    ...
                    print(f"Stage 1: Batch {i_batch + 1}/{num_batches} | Loss computed: {loss.item():.4f}")

                elif stage == 0:
                    loss = -lld
                    loss.backward()
                    print(f"Stage 0: Batch {i_batch + 1}/{num_batches} | Negative log-likelihood loss: {loss.item():.4f}")
                else:
                    raise ValueError('Stage index can either be 0 or 1')
                ...
                if left_batch == 0:
                    ...
                    torch.nn.utils.clip_grad_norm_(model.parameters(), self.hp.clip)
                    optimizer.step()
                    print(f"Optimizer step executed for batch {i_batch + 1}/{num_batches}")
                ...
                if i_batch % self.hp.log_interval == 0 and i_batch > 0:
                    ...
                    avg_nce = nce_loss / proc_size
                    avg_ba = ba_loss / proc_size
                    print(f'Epoch {self.epoch} | Batch {i_batch}/{num_batches} | Time/Batch(ms) {elapsed_time * 1000 / self.hp.log_interval:.2f} | '
                          f'Train Loss {"TASK+BA+CPC" if stage == 1 else "Neg-lld"} {avg_loss:.4f} | NCE {avg_nce:.3f} | BA {avg_ba:.4f}')
                    proc_loss, proc_size = 0, 0
                    nce_loss, ba_loss = 0.0, 0.0
                    start_time = time.time()
        ...
        def evaluate(model, criterion, test=False):
            ...
            print(f"Starting evaluation. Test mode: {test}")
        ...
        for epoch in range(1, self.hp.num_epochs + 1):
            ...
            train(model, optimizer_mmilb, criterion, stage=0)
            train(model, optimizer_main, criterion, stage=1)

            val_loss, results, truths = evaluate(model, criterion)
            print(f"Validation Loss after Epoch {epoch}: {val_loss:.4f}")

            scheduler_mmilb.step(val_loss)
            scheduler_main.step(val_loss)

            results = results.view(-1)
            truths = truths.view(-1)
            mae = eval_mosi(results, truths, True)

            if val_loss < best_valid:
                print(f"New best validation loss: {val_loss:.4f}. Saving model.")
                best_valid = val_loss
                patience = self.hp.patience
                torch.save(self.model, f'{self.hp.model_save_path}/{self.hp.name}.pt')
        ...
        model = torch.load(f'{self.hp.model_save_path}/{self.hp.name}.pt')
